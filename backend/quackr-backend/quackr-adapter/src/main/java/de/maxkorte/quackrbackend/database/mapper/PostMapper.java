...
public class PostMapper {
    ...
    public Post toDomain(PostDTO postDTO) {
        return new Post(postDTO.getId(), postDTO.getTitle(), postDTO.getBody(), userMapper.toDomain(postDTO.getUser()));
    }

    public PostDTO toDTO(Post post) {
        return new PostDTO(post.getId(), post.getTitle(), post.getBody(), userMapper.toDTO(post.getUser()));
    }
